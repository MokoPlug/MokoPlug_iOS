...
@interface MKBMLScanFilterView()<UIGestureRecognizerDelegate>
...
@implementation MKBMLScanFilterView

- (void)dealloc{
    NSLog(@"MKBMLScanFilterView销毁");
}
...
+ (void)showSearchKey:(NSString *)searchKey
    ...
    MKBMLScanFilterView *view = [[MKBMLScanFilterView alloc] init];
...
- (UIView *)backView{
    if (!_backView) {
        _backView = [[UIView alloc] initWithFrame:CGRectMake(offset_X,
                                                             ...
                                                             backViewHeight)];
        _backView.backgroundColor = RGBCOLOR(242, 242, 242);
    }
    return _backView;
}
...
- (UITextField *)textField{
    if (!_textField) {
        ...
        _textField.frame = CGRectMake(offset_X,
                                      ...
                                      30.f);
        _textField.backgroundColor = RGBCOLOR(224, 224, 224);
        _textField.borderStyle = UITextBorderStyleNone;
        _textField.font = MKFont(13.f);
        _textField.textColor = DEFAULT_TEXT_COLOR;
        _textField.attributedPlaceholder = [MKCustomUIAdopter attributedString:@[@"Filter Name"] fonts:@[MKFont(15.f)] colors:@[RGBCOLOR(128, 128, 128)]];
...
- (UIImageView *)signalIcon{
    if (!_signalIcon) {
        _signalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(offset_X,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _signalIcon.image = LOADICON(@"MKBLEMokoLife", @"MKBMLScanFilterView", @"mk_bml_wifisignalIcon.png");
    }
    return _signalIcon;
}
