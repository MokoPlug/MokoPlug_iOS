...
@implementation MKEnergyTableHeaderViewModel

@end

@interface MKEnergyTableHeaderView ()
...
@implementation MKEnergyTableHeaderView
...
#pragma mark - setter
- (void)setViewModel:(MKEnergyTableHeaderViewModel *)viewModel {
    _viewModel = nil;
    _viewModel = viewModel;
    if (!_viewModel) {
        return;
    }
    self.energyValueLabel.text = (_viewModel.energyValue ? _viewModel.energyValue : @"0");
    self.dateLabel.text = SafeStr(_viewModel.dateMsg);
    self.timeLabel.text = SafeStr(_viewModel.timeMsg);
}
...
- (UILabel *)energyValueLabel {
    if (!_energyValueLabel) {
        ...
        _energyValueLabel.textColor = COLOR_NAVIBAR_CUSTOM;
